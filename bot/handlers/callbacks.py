...
async def traffic_question_callback(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        "🛡️ Ограничение помогает защищать сеть от абьюза и "
        "поддерживать стабильную высокую скорость для всех пользователей.\n\n"
        "📊 Практика показывает, что при активном использовании ВПНа "
        "расход трафика одним пользователем редко превышает "
